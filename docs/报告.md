...
![mvn test 执行结果](./screenshots/11-unit-tests-pass.png)
...
1. DBeaver 打开 C 院 MySQL 连接 → `college_c_edu.sc` 表 → 任意选一行，将 `Grd` 改为 `999` → 保存（MySQL INT 无上限限制，可以写入；但 XSD 要求 `score ≤ 100`，统计时校验失败）
2. 任意院 GUI 点击"集成统计(全院)"

**期望**：弹窗中 C 院显示"不可用（...）"错误信息，A/B 院数据正常
