...
**Student 3（college-c）** 负责 C 院 MySQL 库实现，并贡献了首版 `XmlFrameProtocol`、规范格式 XSD 草案和 C↔规范格式 XSL 验证思路。合并清理后公共协议集中到 `common` 模块，规范 XSD/XSL 与对应测试集中到 `integration-server`。
...
| Common | 新建 common 模块 | 统一维护 XmlFrameProtocol 和 XmlSchemaValidator，删除四/三模块重复副本 |

`XmlFrameProtocol` 与 `XmlSchemaValidator` 已集中到 `common` 模块；规范格式 XSD/XSL 主资源仅由 `integration-server` 持有。今后协议、XSD、XSL 调整只改唯一事实源，避免多模块副本漂移。各院 `AppConfig` 因 properties key 差异（db.* / 自院端口）仍各持一份，是有意保留的"配置入口"边界。
