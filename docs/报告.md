...
<img src="./assets/院系A-ER图.png" alt="院系A-ER图" style="zoom:50%;" />
...
<img src="./assets/院系B-ER图.png" alt="院系B-ER图" style="zoom:50%;" />
...
<img src="./assets/院系C-ER图.png" alt="院系C-ER图" style="zoom:50%;" />
...
sequenceDiagram
    ...
    U->>Bts: DROP|B24000001|AC01
    Bts->>Bdb: DELETE FROM ENROLLMENT
    Bdb-->>Bts: 1 row
    Bts->>IS: INTEGRATED_DROP|B24000001|AC01
...
flowchart TD
    ...
    ERR --> CALLER["集成服务器标记该院'不可用'"]
    OK --> IS["集成服务器收到 XML"]
    IS --> XSL["XSL 变换为规范格式"]
    XSL --> UV["XmlValidator.validateUnified* 统一 XSD"]
    UV -- "通过" --> COUNT["计数并汇入统计报告"]
    UV -- "抛 SAXException" --> CALLER
```
...
<img src="./screenshots/11-unit-tests-pass.png" alt="mvn test 执行结果" style="zoom:50%;" />
...
<img src="./screenshots/03-local-pick-result.png" alt="本院选课成功弹窗" style="zoom:50%;" />
...
<img src="./screenshots/05-cross-enroll-result.png" alt="跨院选课成功弹窗" style="zoom:50%;" />

<img src="./screenshots/08-db-college-a-sc.png" alt="A院Enrollment表新增B学生记录（DBeaver）" style="zoom:50%;" />

<img src="./screenshots/db-college-b-sc.png" alt="B院ENROLLMENT表同步记录跨院选课（双写验证，DBeaver）" style="zoom: 67%;" />
...
<img src="./screenshots/09-db-college-b-enrollment.png" alt="B院ENROLLMENT表含外院学号（DBeaver）" style="zoom:67%;" />
...
<img src="./screenshots/06-cross-drop-result.png" alt="退课成功弹窗" style="zoom:50%;" />
...
<img src="./screenshots/07-integrated-stats.png" alt="集成统计弹窗" style="zoom:67%;" />
...
1. DBeaver 打开 B 院 Oracle 连接 → 编辑 STUDENT 表任意一行，将 `STU_NAME` 改为超过 10 个汉字的字符串（如"这是一个超过十个字的非常非常长的名字"）→ 提交
2. 任意院 GUI 点击"集成统计(全院)"

**期望**：弹窗中 B 院显示"不可用（...）"错误信息，A/C 院数据正常
...
|---|---|---|---|
| A | admin / 123456 | A20240001 | 123456 |
| B | admin / 123456 | B24000001 | 123456 |
| C | admin / 123456 | C20240001 | 000000 |
...
<img src="./screenshots/02-main-course-list.png" alt="B院主界面（课程列表 + 我的选课）" style="zoom:50%;" />

### 跨院选课操作

<img src="./screenshots/04-cross-enroll-dialog.png" alt="填写跨院课号并选择目标学院" style="zoom:50%;" />

### 数据库状态

<img src="./screenshots/10-db-college-c-sc.png" alt="C院sc表（DBeaver）" style="zoom:50%;" />
