...
└── src/main/
    ...
    │   ├── config/   (AppConfig)
    │   ├── db/       (Db)
    │   └── net/      (XmlFrameProtocol — 三个院系共用的协议工具)
    └── resources/
        ├── application.properties
        ├── xsd/college-b/    studentB.xsd / classB.xsd / choiceB.xsd / accountB.xsd
        ├── xsd/integration/  formatStudent.xsd / formatClass.xsd / formatChoice.xsd（规范格式）
        └── xsl/integration/  4 个 XSL（B ↔ 规范格式 双向变换）
```
...
所有协议细节走 `cn.nju.dataintegration.net.XmlFrameProtocol`，请把这个文件按原样复制到集成服务器项目里。
...
我在 `xsl/integration/classB_to_unified.xsl` 里用 `concat('0000', CRS_NO)` 把 B 的 5 位课号
...
- `xsd/integration/formatStudent.xsd` / `formatClass.xsd` / `formatChoice.xsd`
