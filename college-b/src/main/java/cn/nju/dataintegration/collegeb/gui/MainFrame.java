package cn.nju.dataintegration.collegeb.gui;
...
/**
 * 院系 B 主界面。布局沿用 Student 3 的设计，字段标签换 B 中文（专业 / 课程编号 / 课时 / 学分 等）。
 * 学生角色可见跨院选课区（目标院系下拉框 A/C）。
 */
public final class MainFrame extends JFrame {
    private final String role;
    private final String userId;
    private final CollegeBClient client;
    ...
    private final JComboBox<String> crossTargetBox = new JComboBox<>(new String[]{"A", "C"});

    public MainFrame(AppConfig config, String role, String userId) {
        this.role = role;
        this.userId = userId;
        this.client = new CollegeBClient(config);
        setTitle("院系 B 化学学院教务系统 — " + role + " — " + userId);
    ...
    private void refreshAll() {
        try {
            ...
            for (String line : courses.split("\n")) {
                ...
                if (r.length >= 7) {
                    courseModel.addRow(new Object[]{r[0], r[1], r[2], r[3], r[4], r[5], r[6]});
                }
            }
            if ("STUDENT".equals(role)) {
