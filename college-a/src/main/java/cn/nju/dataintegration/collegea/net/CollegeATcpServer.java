 ...
 * GUI 侧负责把消息呈现给用户。
 */
public final class CollegeATcpServer implements Runnable {
    private final AppConfig config;
    private final Db db;
    private final CollegeARepository repo = new CollegeARepository();

    public CollegeATcpServer(AppConfig config, Db db) {
        this.config = config;
        this.db = db;
    }
    ...
    public void run() {
        ...
        try (ServerSocket ss = new ServerSocket(port)) {
            System.out.println("[CollegeATcpServer] listening on " + port);
            while (!Thread.currentThread().isInterrupted()) {
                Socket s = ss.accept();
                new Thread(() -> handle(s), "college-a-client").start();
            }
        } catch (IOException e) {
